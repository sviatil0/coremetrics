...
constexpr float STUB_CPU_PCT = 42.0f;
constexpr float STUB_MEM_PCT = 58.0f;
constexpr std::size_t STUB_PROCESS_COUNT = 10;

#ifdef __linux__
static unsigned long long g_lastTotal = 0;
static unsigned long long g_lastIdle = 0;

static bool compareByMemDesc(const ProcessInfo &a, const ProcessInfo &b)
{
...
std::vector<ProcessInfo> SystemMetrics::topProcesses(std::size_t n)
{
    ...
    std::sort(result.begin(), result.end(), compareByMemDesc);
    if (result.size() > n)
    {
        result.resize(n);
    }
#else
    std::size_t count = n < STUB_PROCESS_COUNT ? n : STUB_PROCESS_COUNT;
    for (std::size_t i = 0; i < count; ++i)
    {
        ProcessInfo info;
        info.pid = 1000 + static_cast<int>(i);
        info.name = "stub_proc_" + std::to_string(i);
        info.cpuPct = 10.0f - static_cast<float>(i);
        info.memPct = 20.0f - static_cast<float>(i);
        result.push_back(info);
    }
#endif
